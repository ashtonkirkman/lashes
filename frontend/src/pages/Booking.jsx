...
export default function Booking(){
  ...
  return (
    <section className="max-w-md mx-auto px-6 py-12 bg-ivory">
      <h2 className="text-2xl font-serif text-brown mb-4">Book an Appointment</h2>
      <form onSubmit={submit} className="space-y-3">
        <input required value={form.name} onChange={e=>setForm({...form, name:e.target.value})}
          placeholder="Your name" className="w-full p-3 rounded border border-beige text-brown bg-ivory" />
        <input required value={form.email} onChange={e=>setForm({...form, email:e.target.value})}
          placeholder="Email" className="w-full p-3 rounded border border-beige text-brown bg-ivory" />
  <select required value={form.serviceId} onChange={e=>setForm({...form, serviceId:e.target.value})} className="w-full p-3 rounded border border-beige text-brown bg-ivory">
        ...
        <input required value={form.datetime} onChange={e=>setForm({...form, datetime:e.target.value})}
          type="datetime-local" className="w-full p-3 rounded border border-beige text-brown bg-ivory" />
  <button type="submit" className="w-full py-3 rounded-full bg-blush text-brown font-semibold border border-beige">Request Booking</button>
      </form>
  {status === "sending" && <p className="mt-3 text-sm text-brown/80">Sending...</p>}
